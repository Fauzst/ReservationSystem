...
namespace ReservationSystem.Forms.Customer
{
    ...
    {
        public int accountID_MakeReservation;

        public CustomerReservation(int accountID)
        {
            InitializeComponent();
            accountID_MakeReservation = accountID;
        ...
        {
            Customer_Main customer_Main = new Customer_Main(accountID_MakeReservation);
            customer_Main.Show();
            this.Hide();
        }
        ...
        {
            CustomerProfile customerProfile = new CustomerProfile(accountID_MakeReservation);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
                ...
                try
                {
                    using (MySqlConnection connection = new MySqlConnection(connectionString))
                    {
                        ...
                        using (MySqlCommand command = new MySqlCommand(dateValidationQuery, connection))
                        {
                            ...
                            if (result == null)
                            {
                                string queryGetInfo = "SELECT lastName, firstName FROM customer WHERE accountID = @accountID";
                                string queryAddReservation = "INSERT INTO reservations (dateStart, dateEnd, venue, room, bill, firstName, lastName, reservationStatus, accountID, catering) " +
                                                              "VALUES (@dateStart, @dateEnd, @venue, @room, @bill, @firstName, @lastName, @reservationStatus, @accountID, @catering);";

                                // Get lastName and firstName
                                using (MySqlCommand getInfoCommand = new MySqlCommand(queryGetInfo, connection))
                                {
                                    getInfoCommand.Parameters.AddWithValue("@accountID", accountID_MakeReservation);
                                ...
                                using (MySqlCommand insertReservation = new MySqlCommand(queryAddReservation, connection))
                                {
                                    ...
                                    insertReservation.Parameters.AddWithValue("@accountID", accountID_MakeReservation);
            ...
            {
                MessageBox.Show("Some Fields are not defined! Only Promo Code can be left empty!");
            }
